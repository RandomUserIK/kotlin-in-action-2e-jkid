...
fun Seed.createSeedForType(paramType: KType, isList: Boolean): Seed {
	...
	if (paramClass.isSubclassOf(Map::class)) {
		val typeArguments = paramType.arguments

		if (typeArguments.size < 2) {
			throw JKidException("Map must have two type arguments")
		}

		// Map<K, V> -> [1] is the Value type (V)
		val valueKType = typeArguments[1].type ?: throw JKidException("Map value type cannot be determined")

		return MapSeed(valueKType, classInfoCache)
	}
...
class MapSeed(
	...
	private val valueElements = mutableMapOf<String, Any?>()
	private val seedElements = mutableMapOf<String, Seed>()

	override fun spawn(): Map<String, Any?> = valueElements + seedElements.mapValues { it.value.spawn() }

	override fun createCompositeProperty(propertyName: String, isList: Boolean): JsonObject =
		createSeedForType(elementType, isList).also { seedElements[propertyName] = it }

	override fun setSimpleProperty(propertyName: String, value: Any?) {
		valueElements[propertyName] = value
	}
